...
class DMap extends PureComponent {
  ...
  componentWillUnmount() {
    ...
    // this.removeAllLayer(this.state.allLayers);
  }
  ...
  deleteArea = e => {
    ...
    if (Array.isArray(e.features)) {
      ...
      const geometry = feature?.geometry?.coordinates || [];
      const str = flatArray(geometry);
      callBack && callBack([], 'delete', this.state.selectMap);
    }
      ...
      if (!this.time) {
        this.time = setInterval(() => {
          ...
          this.setStateP({
            ...
            this._addDeckLayer(this.state.deckglLayers);
          });
        }, 1000 / frequencyTime);
        timerSave.push({
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    console.log('asdfasdfmmsadfasdf', layers);
    ...
    if (layers !== nextProps?.layers) {
      //   if (dlayers) {
      //     for (const key in dlayers) {
      //       if (dlayers.hasOwnProperty(key)) {
      //         if (this.map.getLayer(dlayers[key]?.id)) {
      //           this.map.removeLayer(dlayers[key]?.id);
      //         }
      //       }
      //     }
      //     this.setState({
      ...
      //     });
      //   }
      //   if (nextProps?.layers.length <= 0 && this.state.selectMap) {
  ...
  render() {
    ...
    return <div style={{ position: 'relative', width: '100%', height: '100%' }} id="map"></div>;
  }
}
export default DMap;
