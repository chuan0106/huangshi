...
import { DataSearchType } from './dataConstant';
import {
  // FlyToCamera,
  doFortmatChart,
  layerDataGeoJson,
...
export default {
  ...
  effects: {
    ...
    *getAllMapLayerEffect({ payload }, { call, put, select }) {
      ...
      if (!res.data.code) {
        ...
        // 获取图层选中标签
        // yield put({
        //   type: 'themeExploreModel/dasTagListFunEffect',
        //   payload: {
        //     componentId: data?.id,
        //     componentType: data?.componentType,
        //   },
    ...
    *create_das_link_layer_style_effect({ payload }, { call, put, select }) {
      ...
      let res = yield call(buildMapService.create_das_link_layer_style_service, param);
      if (!res.data.code) {
        let data = res?.data?.data;
    ...
    *getMapBodyDataEffect({ payload }, { call, put, select }) {
      ...
      //这里也需要兼容
      let { dataSource, fortmatChart } = oneLayerData.option;
      console.log('fortmatChartrr', fortmatChart?.where?.conditions);
      if (homeModel) {
        let ff = fortmatChart?.where?.conditions?.concat(homeModel?.geoWhere);
        if (ff && ff.length > 0) {
          fortmatChart.where.conditions = ff;
        } else {
          fortmatChart.where.conditions = [];
        }
      }
      console.log('fortmatCwwwhart', fortmatChart);
  ...
  reducers: {
    ...
    setIviewStatePropsReducer(state, action) {
      console.log('setIviewStatePropsReducer')
      state.iviewState = action.payload;
      return { ...state };
    },
  },
