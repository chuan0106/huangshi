...
export function layerProCon(layerCon) {
  ...
  if (layerCon?.zoom) {
    ...
    if (defaultZoom?.maxZoom == 17) {
      defaultZoom.maxZoom = 23;
    }
...
// 组件里面倒序排列
export default function customColor(chartColor, colourReversal) {
...
// 空间里面倒序排列
export function customColor1(chartColor, colourReversal) {
  for (let i = 0; i < chartColor.arrayColors.length; i++) {
    if (chartColor.arrayColors[i].selected === 1) {
      if (colourReversal) {
        let newColorArray = JSON.parse(JSON.stringify(chartColor.arrayColors[i].thisColorAll));
