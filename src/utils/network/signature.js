...
{
    let strParam = _valueToString(params)
    let ret = timestamp + strParam
    return _md5(ret)
}
function _valueToString (val)
{
    if (val instanceof Array)
    {
        ...
        for (let item in val)
        {
            let itemString = _valueToString(val[item])
            content += itemString
        }
        return content
    ...
    {
        let content = ''
        let sdic = Object.keys(val).sort()
        for (let key in sdic)
        {
            let keyValue = sdic[key] + '=' + _valueToString(val[sdic[key]])
            content += keyValue
        }
        return content
    }
