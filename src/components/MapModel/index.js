...
import { IconLayer, ScatterplotLayer, GeoJsonLayer } from '@deck.gl/layers';
...
import huangshiLine from '../../../public/huangshi_city_L.geojson';
// import North from '@/components/North';
import { geoJson, shiGeoJson } from '@/utils/publickPage/geoJson';
import { upLayerData } from '../../utils/commonFun';
...
class MapModel extends React.Component {
    ...
    onLoadFun = (mapObj, initCallback) => {
        ...
        const layer = new GeoJsonLayer({
            ...
            getFillColor: [110, 234, 255, 255],
            getLineColor: [110, 234, 255, 255],
        ...
        if (mapObj) {
            let { deckLayerArr } = this.props.globalMapModel;
            deckLayerArr = upLayerData(deckLayerArr, layer);
            this.props.dispatch({ type: 'globalMapModel/setDeckLayerArr', payload: deckLayerArr });
        }
    ...
    _onViewStateChange ({ viewState }) {
        // 地图限制层级
        if (viewState.zoom <= 14.58531408747953 && viewState.zoom >= 8.462354657997096) {
    ...
    onClickFun = data => {
        ...
        if (object) {
            ...
            if (type === 'Point' || type === 'MultiPoint') {
                let arr = viewObj.project(coordinates);
                console.log(arr, 'dsadasfwewqedas');
                obj = {
                    lngLat: coordinates,
                    x: data.x,
                    y: data.y,
